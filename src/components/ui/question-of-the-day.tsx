...
export function QuestionOfTheDay({ 
  ...
  return (
    <Card className="android-surface border-none shadow-md3-2 rounded-[2.5rem] bg-card overflow-hidden transition-colors duration-300">
      <CardHeader className="p-6 pb-4">
        <div className="flex items-center justify-between">
          ...
          {alreadyClaimedOnMount && (
            <Badge variant="outline" className="h-6 px-3 bg-emerald-500/10 text-emerald-600 border-emerald-500/20 font-black text-[9px] uppercase tracking-widest rounded-full shrink-0">
      ...
      <CardContent className="p-6 space-y-6">
        <div className={cn(
          ...
          (alreadyClaimedOnMount || showResult) ? "bg-muted/10 border-border opacity-80" : "bg-primary/5 border-primary/20 shadow-inner"
        )}>
          ...
          <p className="text-base md:text-lg font-black leading-relaxed text-foreground tracking-tight">
            {question.text}
          </p>
          ...
          {question.options.map((option, index) => {
            ...
            return (
              <motion.button 
                ...
                className={cn(
                  "w-full p-4 rounded-2xl text-left font-bold transition-all border-2 flex items-center gap-4 relative overflow-hidden", 
                  showAsCorrect ? "border-emerald-500 bg-emerald-500/5 text-emerald-700 shadow-sm" : 
                  showAsWrong ? "border-rose-500 bg-rose-500/5 text-rose-700" : 
                  (showResult && !isSelected) ? "border-border/40 opacity-40 grayscale bg-muted/5" : 
              ...
              >
                <div className={cn(
                  ...
                  showAsCorrect ? "bg-emerald-500 border-emerald-500 text-white" : 
                  showAsWrong ? "bg-rose-500 border-rose-500 text-white" : 
          ...
          {showResult && (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    {isCorrect || alreadyClaimedOnMount ? (
                  ...
                  <div>
                    <p className={cn(
                      "font-black text-sm uppercase tracking-[0.1em]", 
                      isCorrect || alreadyClaimedOnMount ? "text-emerald-700" : "text-rose-700"
                    )}>
              ...
              {question.explanation && (
                <div className="pt-4 border-t border-black/5 bg-card/60 p-5 rounded-2xl shadow-sm">
